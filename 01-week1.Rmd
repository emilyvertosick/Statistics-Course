...
So of the `r nrow(lesson1a)` patients (column "n"), the mean age (a type of average, I’ll explain next week) is `r style_sigfig(skim(lesson1a$age)$value[[4]], digits = 4)` (column "mean"), the standard deviation (again, I’ll explain next week) is `r style_sigfig(skim(lesson1a$age)$value[[5]], digits = 4)` (column "sd"). The youngest patient was `r skim(lesson1a$age)$value[[6]]` (column "p0") and the oldest is `r skim(lesson1a$age)$value[[10]]` (column "p100"). 

This simple command gives us our first lesson about the dangers of statistical software: it gives the age to within a few days. So if you were reporting results for a journal, you would never say that mean age was `r style_sigfig(skim(lesson1a$age)$value[[4]], digits = 4)`, you’d probably just say `r round(skim(lesson1a$age)$value[[4]], 0)`.
