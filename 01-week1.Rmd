...
So of the `r nrow(lesson1a)` patients (column "n"), the mean age (a type of average, I’ll explain next week) is `r style_sigfig(skim(lesson1a$age) %>% pull(numeric.mean), digits = 4)` (column "mean"), the standard deviation (again, I’ll explain next week) is `r style_sigfig(skim(lesson1a$age) %>% pull(numeric.sd), digits = 4)` (column "sd"). The youngest patient was `r skim(lesson1a$age) %>% pull(numeric.p0)` (column "p0") and the oldest is `r skim(lesson1a$age) %>% pull(numeric.p100)` (column "p100"). 
