...
\addcontentsline{toc}{section}{Guidelines for presentation of
statistics}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{skim}\NormalTok{(lesson1a)}

\NormalTok{lesson1a }\OperatorTok{%>%}\StringTok{ }\KeywordTok{skim}\NormalTok{()}

\CommentTok{# These also give the same results}
...
\StringTok{  }\CommentTok{# Add a column with totals across all locations}
\StringTok{  }\CommentTok{# "last = TRUE" puts the column on the right side of the table}
...
\NormalTok{        stage }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"T1A"}\NormalTok{, }\StringTok{"T1B"}\NormalTok{, }\StringTok{"T1C"}\NormalTok{) }\OperatorTok{~}\StringTok{ "T1"}\NormalTok{,}
\NormalTok{        stage }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"T3"}\NormalTok{, }\StringTok{"T4"}\NormalTok{) }\OperatorTok{~}\StringTok{ "T3/4"}\NormalTok{,}
...
\StringTok{  }\CommentTok{# Create a new variable called "pright" equivalent to "p"}
\StringTok{  }\CommentTok{# (the pain score for the right wrist (site 2) only)}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}
    ...
    \CommentTok{# By default, any observations that do not fall into the specified conditions}
    \CommentTok{# in the "case_when" statement will be set to missing (NA)}
\NormalTok{  ) }\OperatorTok{%>%}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Create a new variable "c1" which is "1" if meat consumption is high and "0" if it is medium}
\CommentTok{# "c1" is missing for low meat consumption: these patients are left out of the analysis}
...
\KeywordTok{epi.2by2}\NormalTok{(}\KeywordTok{matrix}\NormalTok{(}\KeywordTok{rev}\NormalTok{(}\KeywordTok{table}\NormalTok{(lesson4b}\OperatorTok{$}\NormalTok{c1, lesson4b}\OperatorTok{$}\NormalTok{hbp)), }\DataTypeTok{nrow =} \DecValTok{2}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# "c2" is "1" if meat consumption is high and "0" if it is low}
\CommentTok{# "c2" is missing for medium meat consumption: these patients are left out of the analysis}
...
\KeywordTok{epi.2by2}\NormalTok{(}\KeywordTok{matrix}\NormalTok{(}\KeywordTok{rev}\NormalTok{(}\KeywordTok{table}\NormalTok{(lesson4b}\OperatorTok{$}\NormalTok{c2, lesson4b}\OperatorTok{$}\NormalTok{hbp)), }\DataTypeTok{nrow =} \DecValTok{2}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}
    \DataTypeTok{mutant =}
      \KeywordTok{case_when}\NormalTok{(}
\NormalTok{        gene }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{) }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Use "filter" to exclude patients with outlying disease duration}
\NormalTok{lesson5b <-}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# There are missing values for unemployment,}
\CommentTok{# so we need to indicate that we only want to use "complete observations"}
...
\CommentTok{# Compare who is defined as high risk from the model}
\CommentTok{# with those defined as high risk using clinical criteria}
...
\CommentTok{# Compare who is defined as high risk from the clinical+marker model}
\CommentTok{# with those defined as high risk using clinical criteria}
\KeywordTok{tbl_summary}\NormalTok{(}
...
You can see now that 133 patients who would otherwise be referred to
adjuvant chemotherapy would not using the model, and 345 of those
defined at low risk by the clinical criteria are reclassified as high
risk from the model (presumably because of a high level of the marker.
...
You'll find that imaging is a statistically significant predictor of
recurrence, even after adjusting for stage, grade and the marker, but it
doesn't improve AUC by very much (from 0.798 to 0.800). You could go on
to see that it also doesn't reclassify very well.
