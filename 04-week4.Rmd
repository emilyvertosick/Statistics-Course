...
```{r loadpkgs4, echo = TRUE, eval = FALSE}
...
```{r section4h, warning = FALSE, echo = FALSE, results = "hide"}
...
tbl_subfour_p <-
  tbl_summary(
    ...
    by = "sex",
    type = list(vars(subfourhour) ~ "categorical"),
    statistic = list(vars(subfourhour) ~ "{p}%")
  )
...
```{r section4l, warning = FALSE, include = FALSE}
