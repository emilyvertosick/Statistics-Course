...
This table also tells us that of men, `r inline_text(tbl_sex_p, variable = "pc", level = "0", column = "0")` had no prior chemo, and `r inline_text(tbl_sex_p, variable = "pc", level = "1", column = "0")` had prior chemo.
...
chisq.test(table(lesson2a$subfourhour, lesson2a$sex), correct = FALSE)$p.value
)`.
...
- `r style_sigfig(abs(tbl_marathon$res$ARisk.strata.wald[[1]]))`% more men finished the race in under four hours ("Attrib risk \*" under "Point estimates and 95% CIs"). This is the difference in rates of sub-four hour marathon in men and women.
