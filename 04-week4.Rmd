...
```{r section4h1, echo = FALSE, results = "asis"}
...
One thing you might notice is that when something doesn’t happen very often, the odds and the probability are very similar. So, for example, something with a probability of 5% (0.05) has an odds of `r tbl_probodds %>% filter(prob_event == "5%") %>% pull(odds) %>% style_sigfig(digits = 4)`. This has important implications for interpreting odds ratios. You will often see in the literature something like “the odds ratio for stage was 1.26, so patients with high stage disease had a 26% higher risk of complications”. This statement is wrong because if the odds ratio is 1.26 then the odds are 26% higher, not the risk. The odds ratio and relative risk will only be similar if the probability of an event is low. A doubling of risk from 1% to 2% is an increase in odds from `r tbl_probodds %>% filter(prob_event == "1%") %>% pull(odds) %>% style_sigfig(digits = 4)` to `r tbl_probodds %>% filter(prob_event == "2%") %>% pull(odds) %>% style_sigfig(digits = 4)`, so very close to an odds ratio of 2. But a doubling of risk from 25% to 50% is an odds ratio of 3 (odds of `r tbl_probodds %>% filter(prob_event == "25%") %>% pull(odds) %>% style_sigfig(digits = 4)` to odds of `r tbl_probodds %>% filter(prob_event == "50%") %>% pull(odds) %>% style_sigfig(digits = 4)`).
