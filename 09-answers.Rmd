...
lesson2a %>%
  select(-tm, -id) %>%
  tbl_summary(
    statistic = list(vars(..continuous..) ~ "{mean} ({sd}))"),
    label = list(vars(age) ~ "Age", vars(sex) ~ "Women",
                 vars(rt) ~ "Race time in minutes",
                 vars(shoe) ~ "Favorite Running Shoe")
  )
...
lesson2c %>%
  ...
  tbl_summary(label = list(vars(stage_category) ~ "Stage"))
...
```{r week2l, echo = FALSE, results = "hide"}

tbl_2e <-
  tbl_summary(
    lesson2e %>% select(days),
    statistic = list(vars(days) ~ "{p}%")
  )
...
tbl_cs <- epi.2by2(matrix(rev(table(lesson4a$cs, lesson4a$nv)), nrow = 2))
...
```{r week4f, include = FALSE, warning = FALSE}

tbl_meatrow <-
  tbl_summary(
    lesson4b %>% select(meat, hbp),
    by = "hbp",
    statistic = list(vars(meat) ~ "{p}%"),
    row_percent = TRUE
  )
...
# High vs medium
tbl_himed <- epi.2by2(matrix(rev(table(lesson4b$c1, lesson4b$hbp)), nrow = 2))
...
tbl_hilo <- epi.2by2(matrix(rev(table(lesson4b$c2, lesson4b$hbp)), nrow = 2))
...
tbl_medlo <- epi.2by2(matrix(rev(table(lesson4b$c3, lesson4b$hbp)), nrow = 2))
...
tbl_hbp <-
  ...
  tbl_summary(
    by = "hbp_fac",
    label = list(vars(meat_fac) ~ "Meat Consumption"),
    row_percent = TRUE)
...
tbl_mutant <-
  tbl_summary(
    ...
    by = "toxicity",
    type = list(vars(mutant) ~ "categorical"),
    statistic = list(vars(mutant) ~ "{p}%")
  )

tbl_mutant_epi <- epi.2by2(matrix(rev(table(lesson4c$mutant, lesson4c$toxicity)), nrow = 2))

p_gene_fisher <- fisher.test(table(lesson4c$gene, lesson4c$toxicity))$p.value

```

The results of the study are shown in the table. Very few participants (`r tbl_gene$table_body$stat_0[[4]]`) were homozygous mutant. For the purposes of analysis therefore, data for these participants was combined with those heterozygous for the gene. Patients with any mutant allele (i.e. heterozygous or homozygous mutant) had lower rates of toxicity (`r inline_text(tbl_mutant, variable = "mutant", level = "1", column = "1")` vs `r inline_text(tbl_mutant, variable = "mutant", level = "0", column = "1")`, p=`r style_pvalue(tbl_mutant_epi$res$chisq.crude[[3]])` by chi squared). The relative risk of toxicity is `r style_sigfig(tbl_mutant_epi$res$RR.crude.wald[[1]])` compared to patients with the wild-type (95% CI `r style_ratio(tbl_mutant_epi$res$RR.crude.wald[[2]])`, `r style_ratio(tbl_mutant_epi$res$RR.crude.wald[[3]])`). This conclusion is not sensitive to the method of analysis: analysis of the table keeping participants in three groups gives p=`r style_pvalue(p_gene_fisher)` by an exact test. These findings should be confirmed in a larger series. In particular, it would be interesting to know whether patients who are homozygous mutant are at particularly increased risk.
...
tbl_chemoresponse <-
  tbl_summary(
    ...
    by = "response",
    type = list(vars(group) ~ "categorical"),
    statistic = list(vars(group) ~ "{p}%")
  )

tbl_chemoresponse_epi <- 
  epi.2by2(matrix(rev(table(lesson4d$group, lesson4d$response)), nrow = 2))
...
# gene2 == 0
tbl_gene0 <-
  epi.2by2(matrix(rev(table(lesson4e_gene[[1]]$gene1,
                            lesson4e_gene[[1]]$cancer)), nrow = 2))

tbl_gene1 <-
  epi.2by2(matrix(rev(table(lesson4e_gene[[2]]$gene1,
                            lesson4e_gene[[2]]$cancer)), nrow = 2))
...
rt_tbl1 <-
  ...
  filter(!is.na(wt)) %>%
  tbl_summary(
    label = list(vars(age) ~ "Age (years)", vars(sex) ~ "Female",
                 vars(tr) ~ "Training miles per week",
                 vars(wt) ~ "Weight (kg)", vars(rt) ~ "Race time (minutes)"),
    statistic = list(vars(..continuous..) ~ "{mean} ({sd})")
  )
...
chitable_men <-
  tbl_summary(
    ...
    by = "chemo",
    type = list(vars(response) ~ "categorical"),
    statistic = list(vars(response) ~ "{p}%")
  )
...
tbl_psa <-
  ...
  select(-cancer) %>%
  tbl_summary(
    label = list(vars(psa) ~ "Total PSA (ng/mL)",
                 vars(psan) ~ "Nicked PSA (ng/mL)",
                 vars(psai) ~ "Intact PSA (ng/mL)",
                 vars(psant) ~ "Nicked-to-total ratio")
  )
...
tbl_regression(nont_model,
               exponentiate = TRUE,
               label = list(vars(psa) ~ "Total PSA",
                            vars(psan) ~ "Nicked PSA",
                            vars(psai) ~ "Intact PSA"))
...
ss_6a <-
  matrix(rev(table(lesson6a$cktest, lesson6a$mi)), nrow = 2)

tbl_ss6a <-
  epi.tests(ss_6a)$elements

ss_6b <-
  matrix(rev(table(lesson6b$cktest, lesson6b$mi)), nrow = 2)
...
```{r week6e, echo = FALSE, warning = FALSE, message = FALSE}
...
tbl_recur <-
  tbl_summary(
    ...
    by = "hi_risk",
    type = list(vars(recurrence) ~ "categorical"),
    statistic = list(vars(recurrence) ~ "{p}%")
  )
...
```{r week6j, echo = FALSE, message = FALSE, warning = FALSE}
...
tbl_markermodel <-
  tbl_summary(
    ...
    by = "hi_risk",
    type = list(vars(model_hirisk2) ~ "categorical"),
    statistic = list(vars(model_hirisk2) ~ "{n}")
  )
...
You can see now that `r inline_text(tbl_markermodel, variable = "model_hirisk2", level = "0", column = "1")` patients who would otherwise be referred to adjuvant chemotherapy would not using the model, and `r inline_text(tbl_markermodel, variable = "model_hirisk2", level = "1", column = "0")` of those defined at low risk by the clinical criteria are reclassified as high risk from the model (presumably because of a high level of the marker. You could also do this:
...
```{r week6o, echo = FALSE, message = FALSE}
...
```{r week6q, echo = FALSE, results = "hide"}
...
tbl_imaging <-
  tbl_summary(
    ...
    by = "model_hirisk2",
    type = list(vars(model_hirisk3) ~ "categorical"),
    statistic = list(vars(model_hirisk3) ~ "{n}")
  )

imaging_reclass <-
  as.numeric(tbl_imaging$table_body[2, 5]) +
  as.numeric(tbl_imaging$table_body[3, 4])
...
tbl_summary_7a <-
  tbl_summary(
    ...
    type = list(vars(sex, obstruction, adhesions) ~ "categorical"),
    label = list(vars(sex) ~ "Sex", vars(age) ~ "Age",
                 vars(obstruction) ~ "Obstruction", vars(adhesions) ~ "Adhesions",
                 vars(node4) ~ "Number of nodes")
  )

tbl_cox_7a <-
  coxph(lesson7a_surv ~ sex + age + obstruction + adhesions + factor(node4), data = lesson7a) %>%
  tbl_regression(
    exponentiate = TRUE,
    label = list(vars(sex) ~ "Female", vars(age) ~ "Age (per 1 year)",
                 vars(obstruction) ~ "Obstruction", vars(adhesions) ~ "Adhesions",
                 vars(`factor(node4)`) ~ "Number of nodes"))
...
```{r week7s, echo = FALSE, warning = FALSE, message = FALSE, results = "hide"}
...
The study consisted of `r nrow(lesson7c)` patients, of whom `r sum(lesson7c$died, na.rm = TRUE)` died during the study. Median duration of follow-up for survivors was `r style_sigfig(median_surv_7c)` years. Median survival in the control and 5FU groups was `r style_sigfig(summary(survfit_7c)$table[3, "median"])` and `r style_sigfig(summary(survfit_7c)$table[1, "median"])`, respectively; median survival for the combination group has not been reached (see figure). The overall log-rank test was significant (p=`r style_pvalue(survdiff_p_7c)`), suggesting that the survival is different between groups. In a multivariable Cox regression with group coded as two dummy variables (5FU and Levamisole), 5FU was found to have little effect on survival (HR `r inline_text(lesson7c_cox, variable = "fu")`). Levamisole, however, led to significantly increased survival (hazard ratio `r inline_text(lesson7c_cox, variable = "lev")`).
...
survminer::ggsurvplot(survfit_7c, legend = "bottom")
...
```{r week7v, eval = FALSE, results = "hide"}
