---
title: "Statistics Course Week 2"
...
install.packages(c("tidyverse", "skimr", "janitor", "gmodels"))
library(skimr)
library(janitor)
library(gmodels)
library(tidyverse)
...
The `tabyl` function (from the `janitor` package) gives a frequency table, in this case, the number of men and women.

```{r section2e, warning = FALSE}

tabyl(lesson1a$sex)

```

So there are 181 (47%) women and 205 (53%) men.

### Two-way tables

The `tabyl` function can also give a two-way table, for example, a table that shows where operations were done (remember that the variable `y` gives the part of the hospital) in men and women separately.

```{r section2f, warning = FALSE}

tabyl(lesson1a, sex, y)

```

This shows that, for example, there were 240 operations done at the main campus, 115 of these were done on men and 125 on women.

The `adorn_percentages` function can be added to show the percentage of men who had operations at each site and the percentage of women who had operations at each site. The additional function `adorn_pct_formatting` allows us to format how the percentages are displayed. In this case, `digits = 2` is used so that the percentages are displayed on a scale of 0-100 with two digits after the decimal place.

```{r section2g, warning = FALSE}
...
This shows that, for example, 8% of men and 12% of women had operations at the "Harding" site, and that, in total, 10% of operations were done at this site. 

You can use `adorn_percentages("col")` to get the percentage of operations at each site that were for men and women. `adorn_totals(c("row", "col"))` will also show us the percentage of men and women treated across all sites.
...
This shows that 48% of the operations at the satellite were on men and 52% on women. Overall (look in the "total" column), 47% of the patients treated in the hospital were men and 53% women.

The `count` function with no options will count the number of total observations.

```{r section2i, warning = FALSE}
...
By the way, try `lesson1a %>% tabyl(age)` and see why continuous data are not summarized by tables!
...
```{r section2n, warning = FALSE}

gmodels::ci(lesson1a$age, confidence = 0.90, na.rm = TRUE)
...
```{r section2o, warning = FALSE}

gmodels::ci.binom(lesson1a$sex, na.rm = TRUE)
